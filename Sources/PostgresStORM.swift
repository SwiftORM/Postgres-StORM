...
open class PostgresStORM: StORM, StORMProtocol {
    ...
    /// Alternate initializer, allows supply of a custom PostgresConnect object.
    ...
    func exec(_ statement: String, params: [String]) throws -> PGResult {
        ...
        let result = self.connection.server.exec(statement: statement, params: params)
        
        // set exec message
        errorMsg = self.connection.server.errorMessage().trimmingCharacters(in: .whitespacesAndNewlines)
